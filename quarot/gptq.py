...
def construct_hessian(
    ...
    for idx, X_batch in enumerate(X):
        if len(X_batch.shape) != 3:
            X_batch = torch.unsqueeze(X_batch,dim=0)
...
def get_signals(
    ...
    modules = [
        ...
        layer_adapter.get_mlp_inputs()[0][0],
    ...
    for module, h in zip(modules, hooks):
        if not isinstance(module, list):
            hooks1.append(module.register_forward_hook(h))
        else:
            tmp  = []
            for m in module:
                if not isinstance(m, list):
                    tmp.append(m.register_forward_hook(h))
                else:
                    tmp.append([m_fen.register_forward_hook(h) for m_fen in m])
            hooks1.append(tmp)
    # hooks = hooks1
...
def quantize_model_gptq(
    ...
    for layer_adapter in tqdm(model_adapter.get_layers(), unit="layer", desc="Quantizing layer"):
        ...
        W_upgate = torch.cat([w.weight.data for w in layer_adapter.get_mlp_inputs()[0]], dim=0)
        W_down_proj = layer_adapter.get_mlp_output()[0].weight.data
        ...
        mlp_inputs = layer_adapter.get_mlp_inputs()[0]
        for module, quantized_weight, scale, offset in zip(
            mlp_inputs,
        ...
        set_tensors(layer_adapter.get_mlp_output()[0], Q_down_proj, scale_down_proj, offset_down_proj)
